...
export function AccountNavigation() {
  ...
  return (
    <nav
      ...
      className="flex flex-wrap items-center justify-end gap-2 border-b border-line bg-surface px-3 py-2 text-sm sm:gap-3 sm:px-8"
    >
      {isGuest ? (
        <span className="rounded-full bg-surface-3 px-3 py-1 font-medium text-ink-2">
          Host · jen pro čtení
        </span>
      ) : account.role === "tester" ? (
