  ...
  return (
    <ul className="grid list-none gap-4 p-0 sm:grid-cols-2">
      {practiceCategories.map((category, index) => (
        <li
          className="rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_12px_32px_rgb(15_23_42/0.05)] sm:p-5"
          key={category.title}
        >
          <span aria-hidden="true" className="text-sm font-semibold text-emerald-700">
            0{index + 1}
          </span>
          <h3 className="mt-2 text-lg font-semibold text-slate-950">{category.title}</h3>
          <p className="mt-1 leading-6 text-slate-600">{category.description}</p>
