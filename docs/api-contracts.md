...
Each event also carries `progressGeneration`, bound when the answer is created. Older events without it belong to the initial all-zero generation. `GET /api/v1/me/progress-generation` and `GET /api/v1/auth/me` reveal the current account generation; tester accounts can read it for compatibility sync. `POST /api/v1/me/progress-reset` requires a signed-in user or admin, same-origin CSRF, and explicit UI confirmation; it returns the new generation. Guests and testers cannot reset. A batch containing a valid event from an old generation returns `409 progress_reset` without inserting any item. The browser refreshes generation and clears old local progress before another upload. Attempt pull pages include `progressGeneration` so a reset racing with a pull is detected. See ADR 0009.
