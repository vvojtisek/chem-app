...
- `localStorage` is limited to small, non-sensitive startup preferences. The periodic-table name practice keeps its element selection and mode there under `inorganic.periodic-table-name-practice.selection` and `.mode`, each a Zod-validated object with `schemaVersion: 1`. Unknown element IDs are dropped on read; a missing, corrupt, or unknown-version value, or blocked storage, falls back to the defaults (all groups, no bottom rows, Název → Značka) without an error.
